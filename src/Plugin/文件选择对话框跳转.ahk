...
PluginHelper.addEntryFunc((*) => 文件选择对话框跳转.main()) ; 添加入口函数等待执行
...
class 文件选择对话框跳转 {
    ...
    static jumpToPath(hwnd, path) {
        if (WinExist("ahk_id " hwnd)) {
            WinActivate() ; 激活选择窗口
            ControlFocus("Edit2")
            ControlSend("{F4}", "Edit2")
            Sleep(25)
            ControlSetText(path, "Edit2")
            Sleep(25)
            ControlSend("{Enter}", "Edit2")
            PluginHelper.Utils.tip("文件选择对话框路径跳转", "路径已跳转至:`n`n" path, 2000)
        } else
            PluginHelper.Utils.tip("文件选择对话框路径跳转", "对话框窗口不存在，跳转失败", 2000)
