...
1. 智能模式下搜索结果为符合执行项**匹配条件**的所有结果（具体见5.1 匹配模式），输入内容为空时**不显示任何**执行项。
...
搜索栏输入内容满足**下方左侧**列表中任意表达式时匹配，并且输入内容经**下方右侧**列表相应行**正则替换后传递给启动处理程序**。
...
1. 打开数据目录: 打开用户数据所在目录
2. 导出用户数据: 选择保存目录，将数据目录复制到该目录
3. 导入用户数据: 选择要导入的目录，用此目录内容覆盖数据目录
